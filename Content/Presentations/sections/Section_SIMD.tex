...
\begin{frame}[fragile]{C\texttt{++}26 SIMD}
	The ISO C\texttt{++} standard has data-parallel types (\texttt{SIMD}) (in \emph{C\texttt{++}26}):
...
\begin{frame}[fragile]{C\texttt{++}26 SIMD ABI}
...
\begin{frame}[fragile]{Kokkos SIMD}
  As with the C\texttt{++}26 SIMD type, it takes a data type and ABI
  \begin{code}
     template <class T, class Abi>
     class basic_simd;
  \end{code}
