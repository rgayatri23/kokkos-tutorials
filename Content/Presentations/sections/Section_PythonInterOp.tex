...
% 1) Application in Python, performance critical stuff in C\texttt{++}
...
\begin{frame}[fragile]
  ...
  \begin{itemize}
    \item {Allocating data in Python and viewing it as Kokkos Views in C\texttt{++}.}
    \item {Allocating data in C\texttt{++} and viewing it as Numpy Arrays in Python.}
  \end{itemize}
...
\textbf{Work-flows orchestrated by Python with the heavy lifting in C\texttt{++} is increasing in popularity}
...
\textbf{PyBind11 is a C\texttt{++} template library for mapping C\texttt{++} types and functions to Python}

\begin{itemize}
  \item Despite the syntax of Python having more similarities to C\texttt{++} than C (e.g. classes),
  the most popular implementation of the Python interpreter is written in C (CPython)
  \begin{itemize}
    \item C\texttt{++} code needs to be translated into implementations of the CPython API
...
\begin{itemize}
  ...
  \begin{itemize}
  \item Invoking \lstinline|Kokkos::finalize()| in C\texttt{++} requires all Kokkos data structures to no longer have reference counts
  \item Python scoping rules are quite different than C\texttt{++} scoping rules
...
PYBIND11_MODULE(example, ex) {
  ...
  ex.def(
    ...
    &generate_view,           // C\texttt{++} function
...
def main(args):
    ...
    arr = numpy.array(view, copy=False)
    # should see printout of data set in C\texttt{++} code
    print(arr)
